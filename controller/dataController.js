// const { addDoc, collection, getDocs, doc, setDoc, deleteDoc } = require("firebase/firestore")
...
exports.getData = async (req, res)=>{
    try{
        // belum bisa muncul data nya
        const id = req.params.id
        const usersCollection = firestore.collection ('Users');
        const getAllData = await usersCollection.where('id','==', id).get();
        // const getAllData = await getDocs(usersCollection);
        ...
        res.status(200).json({
            status:'Succes',
            message:`Succesfully for get data ${id}`, user 
        })
    }catch(err){
        res.status(500).json({
            status:'Failed',
...
exports.deleteData = async (req,res)=>{
    try{
        ...
        res.status(200).json({
            status:'Succes',
            message:'Succesfully for delete data', usersCollection
        })
    }catch(err){
        res.status(500).json({
            status:'Failed',
